...
describe("balance worker", () => {
  const runBalanceTask = async (
    ...
    const result = await workers.balance(variables, task);
    return result;
  };
  ...
  it("throws error if no pot name is passed", async () => {
    const runWithoutPotName = () => runBalanceTask("");
  ...
  it("throws error if no variable name is passed", async () => {
    const runWithoutVarName = () => runBalanceTask("Savings", "");
  ...
  it("doesn't throw error if no transaction details passed", async () => {
    ...
    const runWithoutTransaction = () =>
      runBalanceTask(undefined, undefined, variableDelta);
...
describe("deposit worker", () => {
  const runDepositTask = async (
    ...
    const result = await workers.deposit(variables, task);
    return result;
  };
  it("returns all passed variables", async () => {
  ...
  it("calls monzo.deposit with user, pot, and amount", async () => {
    ...
    expect(monzo.deposit.mock.calls[0]).toEqual(
      expect.arrayContaining([mockVariables.user, "Savings", 150])
    );
  });
  ...
  it("throws error if deposit amount is string and not passed in variables", async () => {
    const runWithBadAmountName = () => runDepositTask(undefined, "testVar");
    await expect(runWithBadAmountName).rejects.toThrow(
      "Deposit amount could not be resolved."
    );
  });
  it("throws error if no pot name is passed", async () => {
    const runWithoutPotName = () => runDepositTask("");
    await expect(runWithoutPotName).rejects.toThrow(
      "Deposit task missing pot."
    );
  });
  it("throws error if no variable name is passed", async () => {
    const runWithoutVarName = () => runDepositTask("Savings", "");
    await expect(runWithoutVarName).rejects.toThrow(
  ...
  it("throws error if no user provided in variables", async () => {
    const runWithoutUser = () =>
      runDepositTask("Savings", 150, { user: undefined });
  ...
  it("doesn't throw error if no transaction details passed", async () => {
    ...
    const runWithoutTransaction = () =>
      runDepositTask(undefined, undefined, variableDelta);
    await expect(runWithoutTransaction).not.toThrowError();
  });
  it("throws an error when macroName is not present in variables", async () => {
    const runWithoutMacroName = () =>
      runDepositTask(undefined, undefined, { macroName: undefined });
...
describe("withdraw worker", () => {
  const runWithdrawTask = async (
    ...
    const result = await workers.withdraw(variables, task);
    return result;
  };
  it("returns all passed variables", async () => {
  ...
  it("throws error if withdraw amount is string and not passed in variables", async () => {
    const runWithBadAmountName = () => runWithdrawTask(undefined, "testVar");
    await expect(runWithBadAmountName).rejects.toThrow(
      "Withdraw amount could not be resolved."
    );
  });
  it("throws error if no pot name is passed", async () => {
    const runWithoutPotName = () => runWithdrawTask("");
    await expect(runWithoutPotName).rejects.toThrow(
      "Withdraw task missing pot."
    );
  });
  it("throws error if no variable name is passed", async () => {
    const runWithoutVarName = () => runWithdrawTask("Savings", "");
    await expect(runWithoutVarName).rejects.toThrow(
  ...
  it("throws error if no user provided in variables", async () => {
    const runWithoutUser = () =>
      runWithdrawTask("Savings", 150, { user: undefined });
  ...
  it("doesn't throw error if no transaction details passed", async () => {
    ...
    const runWithoutTransaction = () =>
      runWithdrawTask(undefined, undefined, variableDelta);
    await expect(runWithoutTransaction).not.toThrowError();
  });
  it("throws an error when macroName is not present in variables", async () => {
    const runWithoutMacroName = () =>
      runWithdrawTask(undefined, undefined, { macroName: undefined });
    await expect(runWithoutMacroName).rejects.toThrow(
...
describe("Notify worker", () => {
  const runNotifyTask = async (
    ...
    const result = await workers.notify(variables, task);
    return result;
  };
  it("returns all passed variables", async () => {
  ...
  it("doesn't throw error if no transaction details passed", async () => {
    ...
    const runWithoutTransaction = () =>
      runNotifyTask(undefined, undefined, undefined, undefined, variableDelta);
    await expect(runWithoutTransaction).not.toThrowError();
  });
});
